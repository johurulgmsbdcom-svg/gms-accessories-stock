...
begin
  ...
  -- item এর বর্তমান unit_price নেওয়া
  select unit_price into item_price from items where id = txn.item_id;

  -- stock reverse করা (RCVD delete = stock কমবে, DLVD delete = stock বাড়বে)
  if txn.type = 'RCVD' then
    update items 
    set current_stock = current_stock - txn.qty,
        current_value = (current_stock - txn.qty) * item_price
    where id = txn.item_id;
  elsif txn.type = 'DLVD' then
    update items 
    set current_stock = current_stock + txn.qty,
        current_value = (current_stock + txn.qty) * item_price
    where id = txn.item_id;
...
CREATE OR REPLACE TRIGGER "trg_after_txn" AFTER INSERT ON "public"."transactions" FOR EACH ROW EXECUTE FUNCTION "public"."fn_update_item_after_txn"();



CREATE OR REPLACE TRIGGER "trg_before_txn" BEFORE INSERT ON "public"."transactions" FOR EACH ROW EXECUTE FUNCTION "public"."fn_apply_transaction"();
